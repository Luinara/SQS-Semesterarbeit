...
Im Frontend werden an einigen Stellen kurzlebige IDs erzeugt, zum Beispiel fuer
...
weil ein Pseudozufallszahlengenerator fuer sicherheitsrelevante Werte ungeeignet
waere. Um den Hotspot technisch zu vermeiden und eine robuste Standardregel zu
haben, nutzt die App fuer zufaellige ID-Anteile `crypto.randomUUID()`.
...
- Kollisionen waeren fuer die Anwendung nicht kritisch; im schlimmsten Fall
...
Der Security Hotspot wurde geprueft. Die urspruengliche Verwendung von
`Math.random()` war nicht fuer Kryptografie gedacht, wurde aber durch
