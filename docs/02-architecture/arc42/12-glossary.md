# Glossar

| Begriff | Bedeutung |
| --- | --- |
| PokeHabit | Name der gamifizierten Self-Care-App. |
| Quest | Tagesaufgabe, die Punkte und Fortschritt geben kann. |
| Quality Gate | Pflichtchecks, die für den Abgabestand grün sein sollen. |
| Quality Hub | Docker-Dashboard für Quality-Report, Logs und HTML-Reports. |
| Quality Runner | Einmaliger Docker-Container, der Backend-, Frontend-, Security-, Coverage- und E2E-Checks ausführt. |
| Trainingspunkte | Punkte aus Quests, die für Pokemon-Training genutzt werden. Technisches Feld: `pendingFeedPoints`. |
| Motivation | Pflegewert des Pokemon-Partners. Technisches Feld: `happiness`. Sinkt bei Inaktivität oder über den manuellen Testbutton. Bei 0 Motivation wird weiterer Verlust auf Wachstum/XP angerechnet; Quests werden dadurch nicht blockiert. |
| Wachstum | Fortschritt Richtung naechstes Pokemon-Level. Technisches Feld: `growth`. |
| Wasserstand | Getrunkene Wassermenge, gemessen in Millilitern. |
| Starter-Pokemon | Pokemon-Partner aus der Registrierung. |
| Session-Cookie | Cookie für die Backend-Session. |
