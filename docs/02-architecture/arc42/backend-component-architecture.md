...
* externe Backend-Dienste wie die PokeAPI austauschbar angebunden werden,
...
| Services        | Enthalten die zentrale Anwendungslogik für Authentifizierung, Aufgaben, Wassertracking, Spielstand und Pokémon-Fortschritt. |
...
| Externe Clients | Kapseln den Zugriff auf externe Backend-Dienste wie die PokeAPI.                                                            |
...
| Game State     | Zusammenführung von Aufgaben, Pflegewerten, Pokémon-Zustand und Fortschritt für das Dashboard. |
| Pokémon        | Starter-Pokémon, Ei-Zustand, Level, XP, Entwicklung und PokeAPI-Fallback.                      |
| Persistence    | Speicherung und Abfrage von Nutzern, Tasks, UserTasks, UserStats und Pokémon-Stammdaten.       |
...
| PokemonController                                   | Primärer Adapter für Pokémon- und Fortschrittsdaten.                    |
...
| PokemonService                                      | Anwendungskern für Pokémon-Fortschritt, Starterdaten und Fallbacklogik. |
| UserRepository / TaskRepository / PokemonRepository | Sekundäre Adapter für Datenbankzugriffe.                                |
| PokeAPI-Client                                      | Sekundärer Adapter für externe Pokémon-Daten.                           |
...
| PokeAPI-Client-Tests          | Timeout, Fehlerfälle und lokaler Fallback über HTTP-Stub.    |
...
| Produktiver externer API-Zwang              | Die PokeAPI ist angebunden, aber durch lokale Fallback-Daten abgesichert.              |
